...
CMD ["sh", "-c", "uv run python manage.py migrate --no-input && uv run python manage.py import_price_grid && uv run gunicorn signals.wsgi:application --bind 0.0.0.0:8000 --workers 2 --timeout 60"]
